...
def render_realtime_simulator():
    ...
    # -----------------------------------------------------
    # לשונית 1: תיק ההשקעות שלי + גרף עוגה
    # -----------------------------------------------------
    with tab_portfolio:
        ...
        if ticker_1:
            ...
            if not df1.empty and stock_obj1 and len(df1) > 20:
                ...
                with tc2:
                    ...
                    if st.button("🟢 בצע פקודת קנייה", use_container_width=True, key="buy_btn"):
                    ...
                    if st.button("🔴 בצע פקודת מכירה", use_container_width=True, key="sell_btn"):
                ...
                fig.update_layout(xaxis_rangeslider_visible=False, template="plotly_dark", height=320)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.error("הזן סימול מניה תקין בסרגל הצדי כדי להציג את חדר המסחר.")

    # -----------------------------------------------------
    # לשונית 3: היסטוריית פעולות ורשימת מעקב
    # -----------------------------------------------------
    with tab_history:
        st.subheader("⭐ מניות במעקב (Watchlist)")
        if ticker_1 and st.button("➕/➖ שנה סטטוס מעקב עבור המניה הנוכחית"):
            if ticker_1 not in user_db["watchlist"]: user_db["watchlist"].append(ticker_1)
            else: user_db["watchlist"].remove(ticker_1)
        ...
        else: st.info("לא בוצעו פעולות בחשבון זה.")
